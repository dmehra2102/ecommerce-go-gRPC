...
type OrderServiceClient interface {
	...
	// Server streaming RPC for real-time order tracking
	TrackOrder(ctx context.Context, in *TrackOrderRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[OrderStatusUpdate], error)
}
...
func (c *orderServiceClient) TrackOrder(ctx context.Context, in *TrackOrderRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[OrderStatusUpdate], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &OrderService_ServiceDesc.Streams[0], OrderService_TrackOrder_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[TrackOrderRequest, OrderStatusUpdate]{ClientStream: stream}
...
type OrderService_TrackOrderClient = grpc.ServerStreamingClient[OrderStatusUpdate]
...
type OrderServiceServer interface {
	...
	// Server streaming RPC for real-time order tracking
	TrackOrder(*TrackOrderRequest, grpc.ServerStreamingServer[OrderStatusUpdate]) error
...
func (UnimplementedOrderServiceServer) TrackOrder(*TrackOrderRequest, grpc.ServerStreamingServer[OrderStatusUpdate]) error {
...
func _OrderService_TrackOrder_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(TrackOrderRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(OrderServiceServer).TrackOrder(m, &grpc.GenericServerStream[TrackOrderRequest, OrderStatusUpdate]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type OrderService_TrackOrderServer = grpc.ServerStreamingServer[OrderStatusUpdate]
